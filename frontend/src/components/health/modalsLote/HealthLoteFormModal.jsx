...
export default function HealthLoteFormModal({ 
  ...
  const handleSelectLote = (lote) => {
    setFormData((prev) => ({ ...prev, lote_id: lote._id }));
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!formData.lote_id) {
      alert("Por favor, seleccione un lote válido de la lista desplegable.");
      return;
    }
  ...
  return (
    <Modal
      ...
      description="Completa la información médica del lote"
      footer={formFooter}
      width="max-w-2xl"
    >
              ...
              <ul className="py-1 max-h-48 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] scrollbar-none">
                {lotesFiltrados.length > 0 ? (
                  lotesFiltrados.map((lote) => (
                    ...
                    >
                      <span className="font-semibold">{lote.codigo_lote}</span>
                    </li>
                  ))
                ) : (
