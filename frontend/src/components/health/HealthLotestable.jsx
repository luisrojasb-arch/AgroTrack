...
export default function HealthLotestable({ data = [], onVerTareasClick, userRole }) {
  return (
      ...
      <table className="w-full text-left border-collapse min-w-[800px]">
        <thead>
          <tr className="border-b border-border-agro">
            ...
            <th className="p-4 text-[12px] font-bold text-gray-500 uppercase tracking-wider">Progreso</th>
            <th className="p-4 text-[12px] font-bold text-gray-500 uppercase tracking-wider">Próxima Tarea</th>
          ...
          {data.length > 0 ? (
            data.map((item, index) => (
              <tr key={item.id || index} className="border-b border-border-agro last:border-b-0 hover:bg-gray-50">
                ...
                <td className="p-4 text-sm text-black">{item.progreso || "-"}</td>
                <td className="p-4 text-sm text-black">{item.proxima_tarea || "-"}</td>
            ...
            <tr>
              <td colSpan="6" className="p-6 text-center text-gray-400">No hay registros de salud en lote para mostrar.</td>
            </tr>
          )}
        </tbody>
      </table>
