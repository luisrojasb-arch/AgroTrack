...
export default function InventoryTableControls() {
  ...
  const defaultCategoria = "Todas las categorías";
  
  const urlSearch = searchParams.get("search") || "";
  const urlCategoria = searchParams.get("categoria") || defaultCategoria;
  ...
  // Para el filtro, el select de categorías
  const handleSelectChange = (nuevoValor) => {
    setSelectedCategoria(nuevoValor);

    const params = new URLSearchParams(searchParams.toString());
    
    // Si elige algo distinto a "Todas las categorías", mandamos el parámetro 'categoria'
    if (nuevoValor && nuevoValor !== "Todas las categorías") {
      params.set("categoria", nuevoValor);
    } else {
