...
export default function AnimalSearchInput({
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(async () => {
      ...
      if (valorInicial && searchTerm === valorInicial.label) {
        return;
      }

      setIsLoading(true);
      const res = await getAnimalesAction({
        search: searchTerm,
        sexo: sexoFilter,
        limit: 5,
      });

      if (res.success && res.data?.animales) {
        setResults(res.data.animales);
      } else {
        setResults([]);
      }
  ...
  }, [searchTerm, sexoFilter, valorInicial]);

  const handleSelect = (animal) => {
    const label = `${animal.codigo} ${animal.nombre ? `- ${animal.nombre}` : ""}`;
    setSearchTerm(label);
    setIsOpen(false);
    onSelect(animal);
  };
